...
# Combine all trash wheel datasets, removing rows with no data
...
combined_trash_wheel <- bind_rows(
  ...
  filter(if_any(-c(dumpster, trash_wheel), ~ !is.na(.))) %>%
...
The combined Trash Wheel dataset contains 1032 observations and 15
...
the same episode and series. I also checked whether the merged dataset
contained all the necessary variables from each of the original
datasets.
...
competition across multiple seasons. The dataset contains 573 rows and
...
contestants range from 17 to 71 years old, with an average age of 36.8
years.
...
The average viewership in season 1 was 2.77 million, increasing to an
average viewership of 10.0393 million in season 5.
